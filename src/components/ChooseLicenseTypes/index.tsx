...
export function ChooseLicenseTypes(props: ExtSelectProps<number, LicenseType, BaseProps>) {
  const {
    addable = false, multiple,
    placeholder = '请选择', allowClear = true, disabled = false,
    value, onChange
  } = formatExtSelectProps<number, LicenseType, BaseProps>(props, (type: LicenseType) => type.id)
  ...
  const options = useMemo(() => {
    ...
    return [
      ...filtered,
      ...optionForAdd ? [{ label: optionForAdd, instance: null as unknown as LicenseType }] : []
    ]
